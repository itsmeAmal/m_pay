...
<meta charset="utf-8">
	...
	<title>EPAY - View Profile Updates</title>
						...
						<div class="card">
							...
							<div class="card-body">
								<ul class="nav nav-tabs">
									<li class="nav-item"><a href="#basic-tab1" class="nav-link active" data-toggle="tab">Pending requests</a></li>
									<li class="nav-item"><a href="#basic-tab2" class="nav-link" data-toggle="tab">Approved requests</a></li>
								</ul>
								...
								<div class="tab-content">
									...
									<table class="table datatable-basic table-striped " id="mytable">
										...
										<tbody id="">
											<% for(int j = 0; j < profileUpdates.size(); j++){
											
												if(profileUpdates.get(j).getRequestStatus().equals("pending")){
													
												if( profileUpdates.get(j).getNewStatus()==2){
													updateStatus="Active";
												}
												if( profileUpdates.get(j).getNewStatus()==3){
													updateStatus="Deactive";
												}
												if( profileUpdates.get(j).getNewStatus()==4){
													updateStatus="Blocked";
												}
												if( profileUpdates.get(j).getNewStatus()==5){
													updateStatus="Suspended";
												}
												
												if(profileUpdates.get(j).getPreStatus()==2){
													preStatus="Active";
												}
												if(profileUpdates.get(j).getPreStatus()==3){
													preStatus="Deactive";
												}
												if(profileUpdates.get(j).getPreStatus()==4){
													preStatus="Blocked";
												}
												if(profileUpdates.get(j).getPreStatus()==5){
													preStatus="Suspended";
												}
											%>
											<tr>
												<td><%=profileUpdates.get(j).getAccNo() %></td>
												<td><%=profileUpdates.get(j).getName() %></td>
												<td><%= profileUpdates.get(j).getNic() %></td>
												<td hidden><%= profileUpdates.get(j).getAddress() %></td>
												<td hidden> <%=profileUpdates.get(j).getGender() %></td>
												<td hidden><%=profileUpdates.get(j).getDob() %></td>
												<td><%=profileUpdates.get(j).getMobileNumber() %></td>
												<td hidden><%= profileUpdates.get(j).getNominee() %></td>
												<td hidden><%= profileUpdates.get(j).getNomineeNic() %></td>
												<td><%=preStatus%></td>
												<td><%=updateStatus %></td>
												<td><%= profileUpdates.get(j).getRequestStatus() %></td>
							...
							<tbody id="">
								<% for(int j = 0; j < profileUpdates.size(); j++){
									...
									if(profileUpdates.get(j).getRequestStatus().equals("Approved")){
					...
					<div class="modal-dialog">
						<div class="modal-content">
							...
							<div class="modal-body">
									<form action="notificationContorller.jsp" id="frmShowData" method="post">
											<div class="row">											
												<div class="col-md-6">
													<fieldset>																		
														...
														<div class="row">
															<div class="col-md-6">
																<div class="form-group">
																	...
																	<input type="text" class="form-control" placeholder="previous status" id="user_preStatus" value="<%-- <%= User_nominee%> --%>"  name="search_Prestatus"  readonly>
																</div>
															</div>
															<div class="col-md-6">
																<div class="form-group">
																	<label>Updated Status</label>
																	<input type="text" class="form-control" placeholder="new status" id="user_newStatus" value="<%-- <%= User_nominee%> --%>"  name="search_Newstatus"  readonly>
																</div>
															</div>
														</div>
